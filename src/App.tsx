...
import { Tv, Flame, Heart, Info, Globe, Shield, RefreshCw, Layers, Sparkles, Check } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 font-sans selection:bg-amber-500 selection:text-neutral-950 pb-20 transition-colors duration-500 relative">
        ...
        <header className={`flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-neutral-900/80 transition-opacity duration-300 ${
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="hidden sm:flex items-center gap-1 text-[10px] font-bold bg-amber-500/10 text-amber-400 border border-amber-500/20 px-2 py-0.5 rounded-full">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <button
              ...
              className="px-3 py-2 bg-gradient-to-r from-amber-500/20 to-yellow-500/20 hover:from-amber-500/30 hover:to-yellow-500/30 text-amber-400 border border-amber-500/30 hover:border-amber-500/50 rounded-xl font-bold text-[10px] tracking-wide flex items-center gap-1.5 cursor-pointer transition-all hover:scale-[1.03] active:scale-95 shrink-0"
              title="Clique para verificar os sinais e garantir estabilidade"
            >
              <Sparkles className="w-3.5 h-3.5 text-amber-400 animate-pulse" />
              <span>Sinal 9950.000% Estável</span>
            </button>
      ...
      {/* Toast Notification for System Status Optimization */}
      {showToast && (
        <div id="status-optimise-toast" className="fixed bottom-6 right-6 left-6 md:left-auto md:w-[420px] bg-neutral-900/95 backdrop-blur-md border-2 border-amber-500/60 p-4 rounded-2xl shadow-2xl z-50 animate-fade-in flex items-start gap-3">
          <div className="p-2 bg-amber-500/20 border border-amber-500/30 text-amber-400 rounded-xl">
            <Sparkles className="w-5 h-5 animate-bounce" />
          </div>
          <div className="flex-1">
            <h4 className="text-white font-bold text-xs">Sintonizador 9950.000% Ativo!</h4>
            <p className="text-neutral-400 text-[10px] mt-1 leading-relaxed">
              Otimização de sinal ativada com sucesso! Todos os links de transmissão e servidores proxy foram otimizados contra erros 404, quedas de sinal e erros de análise de pacote Android!
            </p>
          </div>
          <button 
            onClick={() => setShowToast(false)}
            className="text-neutral-500 hover:text-white text-xs px-1 cursor-pointer"
          >
            ✕
          </button>
        </div>
